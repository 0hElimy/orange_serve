<template>
    <div>
        ...
        <template>
            <router-link :to="{ path: '/logistics/New'}">
        ...
        <div class="total"> 第{{this.page.current_page}}页 共{{logistics.length}}条记录</div>
        <el-pagination
...
<script>
    export default {
        ...
        methods: {
            init() {
                axios.get(`http://127.0.0.1:8000/admin/logistics?page=${this.page.current_page}`).then(res => {
                    console.log(res)
                    let datas = res.data
                    this.logistics = datas.data
                    this.page = {
                        total: datas.total,
            ...
            change_attr(row, attr) {
                axios.patch(`http://127.0.0.1:8000/admin/logistics/change_attr`, {id: row.id, attr: attr}).then(res => {
            ...
            handleEdit(index, row) {
                axios.get(`http://127.0.0.1:8000/admin/logistics/${row.id}/edit`).then(res => {
                    // console.log(res)
                    this.$router.push({name: 'Logistics_Edit', params: {id: row.id}})
                })
            ...
            handleDelete(index, row) {
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                    ...
                    axios.delete(`http://127.0.0.1:8000/admin/logistics/${row.id}`).then(() => {
